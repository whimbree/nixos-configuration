...

{
  ...
  boot.kernelPackages =
    with builtins; with lib; let
      ...
      customPackages = filterAttrs (name: packages: hasSuffix "_xanmod" name && (tryEval packages).success) pkgs.linuxKernel.packages;
      compatiblePackages = filter (packages: compareVersions packages.kernel.version latestCompatibleVersion <= 0) (attrValues customPackages);
